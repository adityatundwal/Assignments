...
public class rough_n_queen {
    ...
    public static void n_queen(boolean [][] board,int row,int tq ){
          if(tq==board.length){
              print_arrangement(board);count++;
              return;
          }
          if(row==board.length)return; 
         for(int col=0;col<board.length;col++){     
        if(isPOssible(board,row,col)){
            board[row][col]=true;   n_queen(board,row+1,tq+1); 
        board[row][col]=false;}
        }
    ...
    public static void print_arrangement(boolean [][] board){
       for(int i=0;i<board.length;i++){
           for(int j=0;j<board[0].length;j++){
               int q=i+1;int pos=j+1;
               if(board[i][j])System.out.print("{"+q+"-"+pos+"} ");
           }
       }
       System.out.print(" ");
    }
